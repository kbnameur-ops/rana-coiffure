  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-ink px-5 py-16">
      <div className="w-full max-w-xl">
        ...
        <h1 className="display mt-4 text-[clamp(2rem,6vw,3rem)] uppercase text-bone">
        ...
        <ol className="mt-10 space-y-6 border-t border-ink-line pt-8">
          {[
            ...
            <li key={title} className="flex gap-5">
              <span className="display flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gold-soft text-ink">
                {i + 1}
              </span>
              <div>
                <p className="font-semibold text-bone">{title}</p>
                <p className="mt-1.5 text-sm leading-relaxed text-clay">{text}</p>
              </div>
            </li>
          ))}
        </ol>
